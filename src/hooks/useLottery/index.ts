...
export default function useLottery(
  options: ILotteryOptionsPartial
) {
  // 默认配置
   ...
   * 产生随机数
   */
  const randomPrize = () => Math.round(Math.random() * (mergeOptions.total - 1))
  ...
  const run = () => {
    ...
    if((steps > (baseSteps + mergeOptions.total)) && (currentIndex.value === prizeIndex.value)) {
